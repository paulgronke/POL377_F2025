...
vote_by_mail_state <- eavs %>%
  group_by(state_full) %>%
  summarise(
    vote_by_mail_ballots = sum(f1d, na.rm = TRUE),  # Provisional ballots cast
    valid_ballots       = sum(f1a, na.rm = TRUE),  # Total voters
    .groups = "drop"
  ) %>%
  mutate(
    mail_in_rate = (vote_by_mail_ballots / valid_ballots) * 100
  ) %>%
  arrange(state_full) %>%
  rename(State = state_full,
         `Mail-In Ballots Cast` = vote_by_mail_ballots,
         `Valid Ballots Cast` = valid_ballots,
         `Mail-In Usage (%)` = mail_in_rate)

# Display the table
knitr::kable(
  provisional_by_state,
  caption = "Mail-In Ballot Usage by State (EAVS 2024)",
  digits = 2
)
...
mail_in_map_data <- vote_by_mail_state %>%
  mutate(region = tolower(State))

# Join with map data
map_data_joined <- us_states %>%
  left_join(mail_in_map_data, by = "region")

# Plot
ggplot(map_data_joined, aes(long, lat, group = group, fill = `Mail-In Usage (%)`)) +
  geom_polygon(color = "white") +
  coord_fixed(1.3) +
  scale_fill_viridis_c(option = "plasma", na.value = "grey90") +
  labs(
    title = "Mail-In Ballot Usage by State (EAVS 2024)",
